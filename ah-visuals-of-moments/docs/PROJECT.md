...
| `/q/[id]` | Постоянный QR-маршрут (`/q/AH001`) с определением языка (cookie -> Accept-Language -> fallback) и 307-редиректом на каноническую историю |
| `/moments/[slug]` | Утилитарный маршрут быстрого перехода к истории без префикса языка |
| `POST /api/orders` | Публичный API создания быстрого заказа с атомарной бронью |

### 10.2. Жизненный цикл историй и поисковая индексация
| Статус | isIndexable | Прямой URL | QR (/q/[id]) | Sitemap | Витрина / Каталог | Robots |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| `draft` | false | 404 | 404 | Нет | Нет | N/A |
| `review` | false | 404 | 404 | Нет | Нет | N/A |
| `published` | true | 200 | 307 → 200 | **Да** | **Да** | index, follow |
| `published` | false | 200 | 307 → 200 | Нет | **Да** | noindex, follow |
| `archived` | false | 200 | 307 → 200 | Нет | Нет | noindex, follow |

*Примечание:* `archived` ≠ `deleted`. Архивные истории остаются доступными по прямым ссылкам и QR-кодам на напечатанных футболках, но исключаются из выдачи поисковиков (`noindex`), sitemap и каталога витрины.

### 10.3. Административные маршруты (Admin)
| Маршрут | Описание |
| :--- | :--- |
...
- `NEXT_PUBLIC_SITE_URL` — базовый URL сайта (боевой production domain: `https://ah-visuals.com`, локальная разработка: `http://localhost:3000`). Обязателен в production окружении.
